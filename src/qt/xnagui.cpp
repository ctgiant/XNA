 ...
 * The DeOxyRibose Developers 2011-2012
 */
#include "xnagui.h"
...
DeOxyRiboseGUI::DeOxyRiboseGUI(QWidget *parent):
    QMainWindow(parent),
    clientModel(0),
    walletModel(0),
...
{
    resize(850, 550);
    setWindowTitle(tr("DeOxyRibose") + " - " + tr("Wallet"));
#ifndef Q_OS_MAC
...
void DeOxyRiboseGUI::createActions()
{
...
void DeOxyRiboseGUI::createMenuBar()
{
#ifdef Q_OS_MAC
...
void DeOxyRiboseGUI::createToolBars()
{
...
void DeOxyRiboseGUI::setClientModel(ClientModel *clientModel)
{
...
void DeOxyRiboseGUI::setWalletModel(WalletModel *walletModel)
{
...
void DeOxyRiboseGUI::createTrayIcon()
{
    QMenu *trayIconMenu;
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void DeOxyRiboseGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void DeOxyRiboseGUI::optionsClicked()
{
...
void DeOxyRiboseGUI::aboutClicked()
{
    AboutDialog dlg;
...
void DeOxyRiboseGUI::setNumConnections(int count)
{
    QString icon;
    switch(count)
    {
...
void DeOxyRiboseGUI::error(const QString &title, const QString &message, bool modal)
{
    // Report errors from network/worker thread
    if(modal)
    {
...
void DeOxyRiboseGUI::changeEvent(QEvent *e)
{
...
void DeOxyRiboseGUI::closeEvent(QCloseEvent *event)
{
    if(clientModel)
    {
#ifndef Q_OS_MAC // Ignored on Mac
...
void DeOxyRiboseGUI::askFee(qint64 nFeeRequired, bool *payFee)
{
    QString strMessage =
        tr("This transaction is over the size limit.  You can still send it for a fee of %1, "
          "which goes to the nodes that process your transaction and helps to support the network.  "
          "Do you want to pay the fee?").arg(
                DeOxyRiboseUnits::formatWithUnit(DeOxyRiboseUnits::XNA, nFeeRequired));
...
void DeOxyRiboseGUI::incomingTransaction(const QModelIndex & parent, int start, int end)
{
        ...
        notificator->notify(Notificator::Information,
                            (amount)<0 ? tr("Sent transaction") :
                              ...
                              tr("Date: %1\n"
                                 ...
                                 "Address: %4\n")
                              .arg(date)
                              .arg(DeOxyRiboseUnits::formatWithUnit(walletModel->getOptionsModel()->getDisplayUnit(), amount, true))
                              .arg(type)
                              .arg(address), icon);
    }
}

void DeOxyRiboseGUI::gotoOverviewPage()
{
...
void DeOxyRiboseGUI::gotoHistoryPage()
{
...
void DeOxyRiboseGUI::gotoAddressBookPage()
{
...
void DeOxyRiboseGUI::gotoReceiveCoinsPage()
{
...
void DeOxyRiboseGUI::gotoSendCoinsPage()
{
...
void DeOxyRiboseGUI::gotoSignMessageTab(QString addr)
{
...
void DeOxyRiboseGUI::gotoVerifyMessageTab(QString addr)
{
...
void DeOxyRiboseGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void DeOxyRiboseGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
        ...
        if (nValidUrisFound)
            ...
            notificator->notify(Notificator::Warning, tr("URI handling"), tr("URI can not be parsed! This can be caused by an invalid DeOxyRibose address or malformed URI parameters."));
    }

    event->acceptProposedAction();
}

void DeOxyRiboseGUI::handleURI(QString strURI)
{
    // URI has to be valid
...
void DeOxyRiboseGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void DeOxyRiboseGUI::encryptWallet(bool status)
{
    if(!walletModel)
        return;
...
void DeOxyRiboseGUI::backupWallet()
{
...
void DeOxyRiboseGUI::changePassphrase()
{
...
void DeOxyRiboseGUI::lockWalletToggle()
{
    if(!walletModel)
        return;
...
void DeOxyRiboseGUI::unlockWallet()
{
    if(!walletModel)
        return;
    // Unlock wallet when requested by wallet model
...
void DeOxyRiboseGUI::showNormalIfMinimized(bool fToggleHidden)
{
...
void DeOxyRiboseGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void DeOxyRiboseGUI::updateMintingIcon()
{
...
void DeOxyRiboseGUI::updateMintingWeights()
{
