...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    // ppcoin: fees are not collected by miners as in xna
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "DeOxyRibose", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    if (hash > hashTarget && pblock->IsProofOfWork())
        return error("DeOxyRiboseMiner : proof-of-work not meeting target");

    //// debug print
    printf("DeOxyRiboseMiner:\n");
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("DeOxyRiboseMiner : generated block is stale");
        ...
        if (!ProcessBlock(NULL, pblock))
            return error("DeOxyRiboseMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ThreadDeOxyRiboseMiner(void* parg);

static bool fGenerateDeOxyRiboses = false;
static bool fLimitProcessors = false;
static int nLimitProcessors = -1;



void DeOxyRiboseMiner(CWallet *pwallet, bool fProofOfStake)
{
    ...
    while (fGenerateDeOxyRiboses || fProofOfStake)
    {
        if (fShutdown)
            return;
        ...
        while (vNodes.empty() || IsInitialBlockDownload() || pwallet->IsLocked())
        {
            ...
            if (fShutdown)
                return;
            if (!fGenerateDeOxyRiboses && !fProofOfStake)
                return;
        }
        ...
        printf("Running DeOxyRiboseMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadDeOxyRiboseMiner(void* parg)
{
    ...
    try
    {
        ...
        DeOxyRiboseMiner(pwallet, false);
        vnThreadsRunning[THREAD_MINER]--;
    }
    catch (std::exception& e) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(&e, "ThreadDeOxyRiboseMiner()");
    } catch (...) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(NULL, "ThreadDeOxyRiboseMiner()");
    }
    nHPSTimerStart = 0;
    if (vnThreadsRunning[THREAD_MINER] == 0)
        dHashesPerSec = 0;
    printf("ThreadDeOxyRiboseMiner exiting, %d threads remaining\n", vnThreadsRunning[THREAD_MINER]);
}


void GenerateDeOxyRiboses(bool fGenerate, CWallet* pwallet)
{
    fGenerateDeOxyRiboses = fGenerate;
    nLimitProcessors = GetArg("-genproclimit", -1);
    if (nLimitProcessors == 0)
        fGenerateDeOxyRiboses = false;
    ...
    if (fGenerate)
    {
        ...
        printf("Starting %d DeOxyRiboseMiner threads\n", nAddThreads);
        for (int i = 0; i < nAddThreads; i++)
        {
            if (!NewThread(ThreadDeOxyRiboseMiner, pwallet))
                printf("Error: NewThread(ThreadDeOxyRiboseMiner) failed\n");
            Sleep(10);
        }
    }
}
